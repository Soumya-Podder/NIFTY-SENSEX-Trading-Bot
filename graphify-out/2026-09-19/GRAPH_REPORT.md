...
## Corpus Check
- 117 files · ~141,258 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1932 nodes · 3448 edges · 195 communities (180 shown, 15 thin omitted)
...
### Community 1 - "main.py"
Cohesion: 0.07
Nodes (68): get, middleware, post, Request, dataset_metadata(), Header inspection is an eligibility hint, not verified historical coverage., agent_control(), agent_retrain() (+60 more)

### Community 2 - "test_paper.py"
Cohesion: 0.16
Nodes (36): BaseSettings, Settings, parametrize, test_failed_persistence_restores_account_and_prevents_ghost_fill(), account(), contract(), enter(), plan_account() (+28 more)

### Community 3 - "test_strategy_portfolio.py"
Cohesion: 0.09
Nodes (43): Closed-position attribution; counts are not causal proof of learning., summarize_paper_episodes(), Three paper hypotheses, one selector, one atomic portfolio authority., _age_seconds(), assess_specialists(), _finite(), Any, Deterministic specialist-agent evidence for a paper candidate. These are… (+35 more)
...
Cohesion: 0.14
Nodes (9): AutonomousTradingAgent, Start the autonomous agent loop., Stop the autonomous agent., Background learning loop - retrains models periodically., Retrain ML models from latest backtest/paper results., Entry-filter validation cannot authorize an untested exit-policy change., Publish event to telemetry., Research coordinator observing the sole portfolio execution authority. Learning… (+1 more)

### Community 8 - "test_reconstruction.py"
Cohesion: 0.15
Nodes (24): _valid_bar(), dhan_research(), merge_series(), Dhan rolling candles -> fixed-strike intraday research, never verified net…, Quarantine conflicting duplicate candles permanently, independent of fetch…, ResearchReplay, Isolated fixture prices: never imported by application runtime., run_fixture() (+16 more)

### Community 9 - "local_time"
Cohesion: 0.06
Nodes (17): PaperBroker, ExperimentStore, FixedLearning, ForwardComparison, Prospective reference account. No real-order authority or additive profit…, All reference-account writes remain in an isolated namespace., ReferenceEngine, local_time() (+9 more)

### Community 10 - "autonomous_agent.py"
Cohesion: 0.14
Nodes (14): Paper research coordinator. Execution belongs exclusively to the portfolio…, download_index_candles(), main(), Download raw 5-year historical market data from Dhan directly into local…, Download continuous minute candles from Dhan in chunks with permanent SQLite…, current_credentials(), The project's .env is authoritative over inherited, possibly stale credentials., event() (+6 more)
...
Cohesion: 0.12
Nodes (23): metrics(), _agent_trades(), learn_from_outcomes(), outcome_lessons(), _passes(), plan_agent_capabilities(), Every outcome is logged. Promotion requires fresh, full chronological replays., Restrict learning statistics to outcomes with recorded agent context. (+15 more)

### Community 14 - "LearningMonitor"
Cohesion: 0.14
Nodes (10): evidence_fingerprint(), explain_evidence(), LearningMonitor, Read the authoritative file for EVERY call; never retain an expired key., Heartbeat counters are activity, not new learning or changed evidence quality., Exercise the real SDK Runner and tool loop with a deterministic provider stub., test_activity_counters_do_not_become_new_learning_evidence(), test_monitor_history_changes_only_with_evidence() (+2 more)
...
### Community 16 - "ai.py"
Cohesion: 0.14
Nodes (18): extract_features(), Entry-quality learning from causal, net-cost outcomes. No order authority., Caller supplies a completed causal row; missing volume/Greeks stay missing., Source validation. Rolling moneyness must never masquerade as a fixed contract., Read one row per exact option contract/minute, with repeated underlying OHLCV., read_contract_csv(), Only a dated source schedule can price a historical order., Fixed research gates. Passing these gates never proves absence of overfitting. (+10 more)

### Community 17 - "LearningService"
Cohesion: 0.17
Nodes (21): LearningService, build_evidence(), Synthetic fixtures test mechanics, never trading performance evidence., test_net_cost_and_future_feature_exclusions(), test_promotion_waits_for_next_session_and_freezes_across_restart(), test_purged_holdout_not_reused_and_no_promotion_without_full_replay(), test_real_classifier_json_roundtrip_and_missing_features(), test_replay_failure_cannot_promote() (+13 more)

### Community 18 - "jobs.py"
Cohesion: 0.26
Nodes (7): main(), Durable, single-worker backtests; browser lifetime never owns a running job., attribution(), report_from_run(), Disjoint session windows, not a row slice that splits concurrent indices., validation_windows(), test_new_daily_budget_and_net_profit_target()
...
Cohesion: 0.10
Nodes (9): Exception, DhanMarketData, exchange_timestamp(), Any, Seed latest values from one broker quote call; live ticks replace them., Dhan MarketFeed adapter. No tick is manufactured when the feed is down., test_websocket_packets_are_bounded_and_do_not_write_database(), test_dhan_quote_timestamp_uses_day_first_and_requires_actual_timestamp() (+1 more)

### Community 21 - "BacktestJobs"
Cohesion: 0.11
Nodes (10): BacktestJobs, dhan_plan_research(), Audit the real baseline without laundering rolling offsets into contracts., CostModel, Standard Dhan index option round-trip charges (₹40 brokerage + STT + turnover +…, execution_context(), plan_protection(), Freeze an observed option-structure stop, never adjust it to fit risk. (+2 more)
...
Cohesion: 0.12
Nodes (20): Enum, str, Decision, Direction, ExpectancyResult, FeatureSnapshot, OptionContract, BaseModel (+12 more)

### Community 26 - "paper_engine.py"
Cohesion: 0.16
Nodes (13): ExpectancyEngine, Broker-sourced charges and empirical expectancy; no invented fee percentages., Observed NET outcomes; fees are already included and never deducted twice.…, RiskDecision, available_risk(), One policy for paper and historical replay; data/fill adapters supply…, RiskEngine, rollback_policies() (+5 more)
...
Cohesion: 0.19
Nodes (16): Chronological, shared-cash replay. Every position retains a fixed contract ID., historical_signals(), Causal portfolio-rule replay with the same seven-day warmup as the live scanner., add_features(), adx(), atr(), ema(), rsi() (+8 more)
...
### Community 35 - "test_estimated_exits.py"
Cohesion: 0.18
Nodes (14): estimate_gap_exit(), Exploratory missing-price liquidation; never an observed candle or live fill., Use only the preceding minute, then liquidate instead of inventing a path., history_row(), Read models for reports: retain audit data without presenting partial profit as…, report_view(), test_estimation_liquidates_gap_and_never_trains(), test_haircut_sensitivity_uses_same_observation() (+6 more)

### Community 36 - "QuoteRecorder"
Cohesion: 0.17
Nodes (6): Path, QuoteRecorder, Bounded, asynchronous recording of normalized observations, never credentials., test_failed_writer_stops_accepting_observations(), test_queue_overflow_and_storage_limit_are_not_silent(), test_quote_round_trip_is_durable_and_does_not_record_credentials()

### Community 38 - "Behavioral Guidelines"
Cohesion: 0.33
...
Cohesion: 0.24
Nodes (6): metrics(), MLTradeQualityModel, number(), One fixed model/threshold, purged 70/30 day split, optional FULL replay. A…, Regularized logistic probability estimate; portable JSON, no pickle loading., scope()

### Community 44 - "test_learning_validation.py"
Cohesion: 0.47
...
### Community 53 - "now_ist"
Cohesion: 0.11
Nodes (11): AgentState, Main agent loop - runs every 2 seconds during market hours., Pre-market preparation: load models, validate data, check risk limits., Observe decisions; only the shared engine is allowed to execute them., End of session processing., Run backtest on recent data to validate strategies., Persistent agent state across sessions., Load persistent agent state. (+3 more)
...
Cohesion: 0.12
Nodes (22): datetime, Causal paper exit state. A stop is a request, never a guaranteed fill., Persistable state; only fresh observed bids may call this function. ATR must…, update_exit(), calendar_info(), Published NSE derivatives closures used as a shared portfolio entry gate. This…, plan_exit(), Shared ordered exit policy; the execution adapter supplies only observed values. (+14 more)
...
### Community 148 - "Behavioral Guidelines"
Cohesion: 0.25
...
## Knowledge Gaps
- **840 isolated node(s):** `name`, `private`, `version`, `type`, `dev` (+835 more)
...
- **Why does `Store` connect `Store` to `main.py`, `test_paper.py`, `test_estimated_exits.py`, `test_jobs.py`, `AutonomousTradingAgent`, `test_reconstruction.py`, `local_time`, `autonomous_agent.py`, `main`, `test_backtest.py`, `LearningMonitor`, `LearningService`, `jobs.py`, `now_ist`, `audit_individual_agents`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `now_ist()` connect `now_ist` to `main.py`, `test_strategy_portfolio.py`, `AutonomousTradingAgent`, `local_time`, `autonomous_agent.py`, `test_backtest.py`, `LearningMonitor`, `ai.py`, `LearningService`, `jobs.py`, `DhanMarketData`, `DhanGateway`, `Store`, `paper_engine.py`, `audit_individual_agents`, `MultiStrategyPaperEngine`, `QuoteRecorder`, `PaperEngine`, `MLTradeQualityModel`, `datetime`?**
