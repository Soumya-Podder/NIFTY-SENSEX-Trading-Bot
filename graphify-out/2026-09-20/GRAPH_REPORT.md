...
## Corpus Check
- 124 files · ~146,425 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 10 file(s) not represented in the graph (top: .bat 4, (none) 3, .css 2)

## Summary
- 2086 nodes · 3765 edges · 210 communities (181 shown, 19 thin omitted)
...
Cohesion: 0.05
Nodes (29): dataset_path(), BacktestJobs, Resolve a user-selected range without treating an explicit zero year as one…, resolve_backtest_budgets(), resolve_backtest_start(), DhanGateway, One rate-limited, cached data adapter shared by paper and research., Reuse overlapping sourced responses; fetch only uncovered date intervals. (+21 more)

### Community 3 - "test_reconstruction.py"
Cohesion: 0.09
Nodes (24): merge_series(), Quarantine conflicting duplicate candles permanently, independent of fetch…, ResearchReplay, Isolated fixture prices: never imported by application runtime., run_fixture(), signal(), series(), test_bad_arrays_are_not_padded() (+16 more)
...
Cohesion: 0.19
Nodes (23): evaluate_strategies(), candidate(), executable(), frame_fixture(), portfolio_account(), parametrize, Synthetic fixtures stay in temporary databases; never loaded into the paper…, test_data_defects_block_every_strategy() (+15 more)
...
Cohesion: 0.06
Nodes (70): get, middleware, post, Request, dataset_metadata(), Header inspection is an eligibility hint, not verified historical coverage., agent_control(), agent_retrain() (+62 more)
...
Cohesion: 0.13
Nodes (13): PaperBroker, Prospective reference account. No real-order authority or additive profit…, local_time(), quote_is_fresh(), Broker, parametrize, Isolated lifecycle evidence; fake accounts never reach the live broker., running() (+5 more)

### Community 16 - "test_backtest.py"
Cohesion: 0.09
Nodes (29): _agent_trades(), learn_from_outcomes(), _learn_from_outcomes(), outcome_lessons(), _passes(), plan_agent_capabilities(), Every outcome is logged. Promotion requires fresh, full chronological replays., Restrict learning statistics to outcomes with recorded agent context. (+21 more)

### Community 17 - "strategy_portfolio.py"
Cohesion: 0.23
Nodes (10): closed_session(), evaluate_completed_bars(), rank_opportunities(), Frozen, causal paper hypotheses. Scores are not probabilities of profit., Rank only eligible offers; statistical support precedes observation economics.…, Return the current day's deterministic strategy preference order., Shared rules for live prefixes and a validated, causal historical prefix.…, regime_strategy_policy() (+2 more)

### Community 18 - "LLMClient"
Cohesion: 0.20
Nodes (6): AsyncClient, create_llm_client(), LLMClient, Async OpenRouter client with retries, timeout, and structured output parsing., Call OpenRouter chat completion with retries. Returns parsed JSON or raises., Factory: returns LLMClient if enabled and configured, else None.
...
Cohesion: 0.09
Nodes (22): main(), Source validation. Rolling moneyness must never masquerade as a fixed contract., Chronological, shared-cash replay. Every position retains a fixed contract ID., Durable, single-worker backtests; browser lifetime never owns a running job., metrics(), dhan_plan_research(), dhan_research(), Dhan rolling candles -> fixed-strike intraday research, never verified net… (+14 more)

### Community 23 - "LearningService"
Cohesion: 0.15
Nodes (17): LearningService, parametrize, test_incomplete_or_estimated_reports_do_not_fit_models(), test_net_cost_and_future_feature_exclusions(), test_promotion_waits_for_next_session_and_freezes_across_restart(), replay(), test_purged_holdout_not_reused_and_no_promotion_without_full_replay(), test_replay_failure_cannot_promote() (+9 more)
...
### Community 27 - "opening_range_retest"
Cohesion: 0.23
Nodes (13): historical_signals(), Causal portfolio-rule replay with the same seven-day warmup as the live scanner., add_features(), adx(), atr(), ema(), rsi(), opening_range_retest() (+5 more)

### Community 28 - "get_analysts"
Cohesion: 0.11
Nodes (10): get_analysts(), HypothesisGenerator, Post-trade thesis validation and exit quality analysis. LLM-enhanced when…, Research hypothesis generation from analysis. LLM-enhanced when configured., Factory returning appropriate analyst instances based on LLM config., _StubFailureAnalyst, _StubHypothesisGenerator, _StubMarketAnalyst (+2 more)

### Community 29 - "observed_session.py"
Cohesion: 0.13
Nodes (29): Read one row per exact option contract/minute, with repeated underlying OHLCV., read_contract_csv(), _valid_bar(), broker_cost_receipts(), candles(), download(), fee_scenario(), fetch() (+21 more)

### Community 30 - "._run"
Cohesion: 0.19
Nodes (11): archive_manifest(), before_commit(), cancelled(), ml_replay(), progress(), replay(), download_history(), Archive source observations only; never run a strategy or update learning. (+3 more)
...
### Community 32 - "test_adaptive_learning.py"
Cohesion: 0.30
Nodes (10): Causal paper exit state. A stop is a request, never a guaranteed fill., Persistable state; only fresh observed bids may call this function. ATR must…, update_exit(), plan_exit(), Shared ordered exit policy; the execution adapter supplies only observed values., position(), Synthetic fixtures test mechanics, never trading performance evidence., test_completed_stall_requires_contiguous_bars() (+2 more)

### Community 33 - "now_ist"
Cohesion: 0.20
Nodes (4): MultiStrategyPaperEngine, execution_health(), Worker liveness is independent of market-data availability and strategy edge., now_ist()

### Community 34 - "PaperEngine"
Cohesion: 0.21
Nodes (4): ExpectancyEngine, Observed NET outcomes; fees are already included and never deducted twice.…, PaperEngine, test_missing_depth_at_1505_keeps_pending_position()
...
Cohesion: 0.24
Nodes (7): extract_features(), ratio(), MLTradeQualityModel, number(), Regularized logistic probability estimate; portable JSON, no pickle loading., Caller supplies a completed causal row; missing volume/Greeks stay missing., test_real_classifier_json_roundtrip_and_missing_features()

### Community 47 - "ai.py"
Cohesion: 0.18
Nodes (13): metrics(), Entry-quality learning from causal, net-cost outcomes. No order authority., One fixed model/threshold, purged 70/30 day split, optional FULL replay. A…, Return the seven auditable gates that precede any model fitting., scope(), training_gate_results(), Fixed research gates. Passing these gates never proves absence of overfitting., replay_evidence() (+5 more)
...
Cohesion: 0.16
Nodes (15): estimate_gap_exit(), Exploratory missing-price liquidation; never an observed candle or live fill., Use only the preceding minute, then liquidate instead of inventing a path., history_row(), Read models for reports: retain audit data without presenting partial profit as…, report_view(), attribution(), report_from_run() (+7 more)
...
Cohesion: 0.15
Nodes (18): BaseSettings, model_validator, download_index_candles(), main(), Download raw 5-year historical market data from Dhan directly into local…, Download continuous minute candles from Dhan in chunks with permanent SQLite…, current_credentials(), The project's .env is authoritative over inherited, possibly stale credentials. (+10 more)

### Community 64 - "session_state"
Cohesion: 0.24
...
Cohesion: 0.15
Nodes (14): datetime, get_autonomous_agent(), Paper research coordinator. Execution belongs exclusively to the portfolio…, Get or create the singleton autonomous agent., execution_context(), plan_protection(), Shared decision stages for historical replay and live-data paper execution., Freeze an observed option-structure stop, never adjust it to fit risk. (+6 more)
...
### Community 209 - "test_execution_integrity.py"
Cohesion: 0.14
Nodes (10): parametrize, Failure injection against isolated E-drive databases; no runtime imports., test_coordinator_has_no_independent_order_authority(), test_coordinator_watchdog_restarts_dead_workers_without_order_authority(), test_failed_persistence_restores_account_and_prevents_ghost_fill(), test_health_detects_stuck_execution_worker(), test_structural_stop_is_not_tightened_to_force_risk_budget(), plan_account() (+2 more)

### Community 211 - "CostModel"
Cohesion: 0.18
Nodes (5): close_position(), close(), CostModel, Standard Dhan index option round-trip charges (₹40 brokerage + STT + turnover +…, Only a dated source schedule can price a historical order.
...
- **Why does `Store` connect `Store` to `test_paper.py`, `DhanGateway`, `test_reconstruction.py`, `AutonomousTradingAgent`, `ForwardComparison`, `main.py`, `local_time`, `test_backtest.py`, `test_learning_monitor.py`, `engine.py`, `LearningService`, `audit_individual_agents`, `observed_session.py`, `test_adaptive_learning.py`, `report_from_run`, `test_runtime.py`, `test_execution_integrity.py`, `datetime`, `test_paper_toggle_is_explicit_and_never_live`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `now_ist()` connect `now_ist` to `DhanGateway`, `AutonomousTradingAgent`, `ForwardComparison`, `main.py`, `local_time`, `test_backtest.py`, `test_learning_monitor.py`, `Store`, `LearningService`, `QuoteRecorder`, `test_adaptive_learning.py`, `PaperEngine`, `summarize_paper_episodes`, `MLTradeQualityModel`, `ai.py`, `report_from_run`, `test_runtime.py`, `session_state`, `datetime`?**
