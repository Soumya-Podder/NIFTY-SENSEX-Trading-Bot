...
## Corpus Check
- 122 files · ~143,022 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 10 file(s) not represented in the graph (top: .bat 4, (none) 3, .css 2)

## Summary
- 2032 nodes · 3606 edges · 212 communities (184 shown, 18 thin omitted)
...
## Communities (212 total, 18 thin omitted)
...
Cohesion: 0.24
Nodes (20): exchange_timestamp(), account(), contract(), enter(), fixture, quote(), Isolated accounting fixtures; none are loaded by the runtime or dashboard., test_account_persists_and_partial_exits_reconcile() (+12 more)

### Community 2 - "test_jobs.py"
Cohesion: 0.12
Nodes (19): Resolve a user-selected range without treating an explicit zero year as one…, resolve_backtest_budgets(), resolve_backtest_start(), historical_gateway(), manager(), parametrize, test_backtest_budgets_are_not_capped_by_paper_settings(), test_completed_report_survives_manager_restart() (+11 more)
...
### Community 10 - "test_backtest.py"
Cohesion: 0.06
Nodes (56): BacktestConfig, BacktestEngine, add_features(), adx(), atr(), ema(), rsi(), PlanRiskPolicy (+48 more)
...
Cohesion: 0.18
Nodes (16): BaseSettings, model_validator, Settings, parametrize, test_failed_persistence_restores_account_and_prevents_ghost_fill(), plan_account(), signal(), quote() (+8 more)
...
### Community 29 - "cli.py"
Cohesion: 0.16
Nodes (14): Path, get_autonomous_agent(), Get or create the singleton autonomous agent., main(), dataset_path(), Read one row per exact option contract/minute, with repeated underlying OHLCV., read_contract_csv(), dhan_research() (+6 more)
...
### Community 34 - "session_state"
Cohesion: 0.29
Nodes (6): session_state(), parametrize, test_engine_cannot_invent_session_exit(), test_session_boundaries(), parametrize, test_exact_session_boundaries()
...
### Community 47 - "PaperEngine"
Cohesion: 0.22
Nodes (4): PaperEngine, event(), Any, Create one JSON-safe, auditable event for a real engine cycle.
...
Cohesion: 0.21
Nodes (8): Paper research coordinator. Execution belongs exclusively to the portfolio…, download_index_candles(), main(), Download raw 5-year historical market data from Dhan directly into local…, Download continuous minute candles from Dhan in chunks with permanent SQLite…, current_credentials(), The project's .env is authoritative over inherited, possibly stale credentials., test_env_rotation_overrides_inherited_old_value()
...
Cohesion: 0.67
Nodes (3): pipeline_from_events(), test_pipeline_carries_event_provenance_for_live_workflow(), test_new_scan_clears_old_agent_decisions()
...
- **Why does `Store` connect `Store` to `test_paper.py`, `test_jobs.py`, `test_reconstruction.py`, `DhanGateway`, `AutonomousTradingAgent`, `ForwardComparison`, `test_backtest.py`, `main.py`, `test_adaptive_learning.py`, `test_paper_toggle_is_explicit_and_never_live`, `autonomous_agent.py`, `test_learning_monitor.py`, `LearningService`, `audit_individual_agents`, `test_runtime.py`, `cli.py`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `now_ist()` connect `now_ist` to `test_reconstruction.py`, `datetime`, `test_strategy_portfolio.py`, `AutonomousTradingAgent`, `ForwardComparison`, `test_backtest.py`, `main.py`, `DhanMarketData`, `post`, `test_learning_monitor.py`, `Store`, `LearningService`, `QuoteRecorder`, `ai.py`, `DhanGateway`, `session_state`, `json_safe`, `test_adaptive_learning.py`, `PaperEngine`, `autonomous_agent.py`, `quote_is_fresh`?**
