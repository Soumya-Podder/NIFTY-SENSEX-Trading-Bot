...
def read_contract_csv(path, config=None, cancel=lambda:False):
    ...
    if raw_required.issubset(data.columns) and not required.issubset(data.columns):
        raise ValueError("Underlying index candles are not historical option prices. Supply observed exact-contract option candles; synthetic option prices cannot validate trading or learning.")
    ...
    for index,((stamp,symbol),group) in enumerate(data.groupby(["timestamp","symbol"],sort=True)):
        ...
        for q in group.to_dict("records"):
            ...
            if has_synthetic_options(q): raise ValueError("Synthetic option data is not verified contract history")
