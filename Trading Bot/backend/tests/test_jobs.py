...
def test_backtest_budgets_are_not_capped_by_paper_settings():
    ...
    assert config["daily_loss_limit"]==pytest.approx(5000*800/600) and config["correlated_risk_limit"]==pytest.approx(5000*600/600)
    assert settings.max_trade_risk_rupees==600 and settings.daily_loss_limit_rupees==800
